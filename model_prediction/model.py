...
class AttenSum(nn.Module):
    ...
    def forward(self, *Z_list):
        ...
        for Z in Z_list:
            ...
            QZ_plus_b = self.Q(Z) + self.b.unsqueeze(0).unsqueeze(0)  # compute Q·Z+b, add batch dims
            q_i = torch.matmul(QZ_plus_b, self.R.weight.t())  # compute Q·Z+b·R^T
            beta = torch.softmax(q_i, dim=1)  # compute beta
            beta = beta.reshape(batch_size, T, nodes)  # reshape to [64, 12, 617]
            betas.append(beta)
        res = []
        for i, beta in enumerate(betas):
            beta_expanded = beta.unsqueeze(-1)  # expand last dim to match x's last dim
...
class MultiGCN(nn.Module):
    def __init__(self, c_in, c_out, dropout, support_list=[6, 6, 6, 4], order=2, fusion='mean'):
        ...
        self.fusion = fusion
        self.max_selections = [0] * len(support_list)  # stats for max fusion selection counts
    ...
    def forward(self, x, supports):
        ...
        # absolute value based max fusion
    ...
    def get_max_selection_stats(self):
        ...
        if total == 0:
            return "Max fusion method not used"
...
class CASTMGCN(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, copy_supports=None, gcn_bool=True, addaptadj=True, aptinit=None,
        ...
        if gcn_bool and addaptadj:
            if aptinit is None:
                if copy_supports is None:  # if None
