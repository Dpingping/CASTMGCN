...
def sym_adj(adj):
    ...
    rowsum[rowsum == 0] = 1e-6
    d_inv_sqrt = np.power(rowsum, -0.5).flatten()  # avoid division by zero
...
def asym_adj_abs(adj):
    adj_abs = np.abs(adj)  # use absolute values for row sums
    adj = sp.coo_matrix(adj)
    adj_abs_sp = sp.coo_matrix(adj_abs)

    rowsum_abs = np.array(adj_abs_sp.sum(1)).flatten()

    d_inv_abs = np.power(rowsum_abs, -1, where=rowsum_abs != 0)
    d_inv_abs[rowsum_abs == 0] = 0.

    d_mat_abs = sp.diags(d_inv_abs)
    # multiply original adj (keeping signs) by D_inv computed from absolute row sums
    return d_mat_abs.dot(adj).astype(np.float32).todense()
...
def process_adj_matrix(adj_mx, adjtype):
    ...
    # convert adj to torch (kept as comment placeholder if needed)
...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    for category in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(dataset_dir, category + '_data.npz'))
