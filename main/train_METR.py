...
from engine_METR import trainer
import pickle
import os
...
parser.add_argument('--addaptadj', default=False, action='store_true', help='whether add adaptive adj')
...
parser.add_argument('--save', type=str, default='../save/METR/trained_model/', help='experiment id')
...
def get_week_matrix(timestamp, matrix_dir='../data/METR-LA/causalgraphs/'):

    try:
        current_date = datetime.fromtimestamp(timestamp)

        # Compute the week of the current date
        ...
        # Try loading the previous week's matrices
        ...
        if os.path.exists(last_week_path):
            ...
            if len(P_est_list) >= 2:
                ...
                if len(matrix1.shape) == 2 and len(matrix2.shape) == 2:
                    # Zero-out small absolute values
                    ratio = 0.3
                    # Threshold (max abs value * ratio)
        ...
        # If not found, use the current week's matrices
        ...
        # Fallback: use a fixed week file if none are found
...
def main():
    ...
    np.random.seed(2023)
    # load data
    device = torch.device(args.device)  # use specified device
    ...
    # Load initial adjacency matrices
    adj_matrices = util.load_adj_matrix('../data/adj_matrices/adj_matrix_la.pkl')
    adj_mx = util.process_adj_matrix(adj_matrices, 'normlap')
    base_adj_mx = adj_mx.copy()  # keep base matrices as backup

    dataloader = util.load_dataset('../data/METR-LA/', args.batch_size, args.batch_size, args.batch_size)
    scaler = dataloader['scaler']

    # Initialize supports list; the first is the fixed adjacency matrix
    adj_matrix = torch.tensor(np.array(adj_mx[0])).to(device)
    adj_matrix = torch.unsqueeze(adj_matrix, 0)
    initial_supports = [adj_matrix]  # only initialize the first fixed adjacency matrix
    print(args)

    if args.randomadj:  # whether to randomly initialize adaptive matrix
        adjinit = None
    else:
        adjinit = initial_supports[0]  # if not random, use the first adjacency as init
    if args.aptonly:  # whether to use adaptive-only adjacency
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            processed_matrix2 = torch.stack([torch.from_numpy(np.array(m)).float() for m in processed_matrix2]).to(
                device)
            # Update supports while keeping the first adjacency matrix fixed
            supports = [adj_matrix, processed_matrix1, processed_matrix2]  # dynamically updated supports

            indices = torch.tensor([0, 2])
            trainx = torch.Tensor(x)
            trainx = torch.index_select(trainx, dim=-1, index=indices).to(device)
            trainx = trainx.transpose(1, 3)
            trainy = torch.Tensor(y[..., 0:1]).to(device)  # use only the first channel
        ...
        for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            # testx = torch.Tensor(x[..., :2]).to(device)
        ...
        if mvalid_loss < best_val_loss:
            ...
            print(f"Saved best model, validation loss: {mvalid_loss:.4f}")
    ...
    outputs = []
    realy = torch.Tensor(dataloader['y_test'][..., 0:1]).to(device)  # use only the first channel
    realy = realy.transpose(1, 3)[:, 0, :, :]

    for iter, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
        # Retrieve timestamps and build matrices
        batch_timestamps = x[:, 0, 0, 1]
        batch_matrices = []
        for timestamp in batch_timestamps:
            timestamp = int(timestamp)
    ...
    pred_all = torch.cat(pred_list, dim=-1)  # [num_samples, num_nodes, 12]
    true_all = torch.cat(true_list, dim=-1)

    # Save denormalized predictions and ground truth
    np.savez(f'{args.save}test_pred_and_true.npz',
             pred=pred_all.cpu().numpy(),
             true=true_all.cpu().numpy())
    print(f'Saved denormalized predictions and ground truth to {args.save}test_pred_and_true.npz')
