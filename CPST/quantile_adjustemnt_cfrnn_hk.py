...
parser.add_argument('--data', type=str, default='../data/HK/', help='data path')
parser.add_argument('--adjdata', type=str, default='../data/sensor_graph/adj_matrix_la.pkl', help='adj data path')
...
parser.add_argument('--save_path', type=str, default='results/cfrnn_hk_90%.npy', help='save path')
...
def main():
    ...
    # Create backup matrix
    backup_matrix = np.zeros((args.num_nodes, args.num_nodes))
    np.fill_diagonal(backup_matrix, 1)  # Create identity matrix as backup
    base_adj_mx = [adj_mx[0], backup_matrix, backup_matrix]  # Use identity matrix as backup
    ...
    val_y = val_y.transpose(1, 3)[:, 0, :, :]  # True y [7066, 617, 12] labels
    calibration_dataset = dataloader['val_loader']
    cal_model = tqa.CFRNN_hk(model)
    ...
    test_y = test_y.transpose(1, 3)[:, 0, :, :]  # True y [7066, 617, 12] labels
